...
export function OtherTenantsSection({ value, onChange, error, tenants = [], disabled = false }: Props) {
    ...
    const handleOtherTenantsInputChange = (inputValue: string): void => {
        onChange(inputValue);
        ...
        <div className="rounded-lg border-l-4 border-l-pink-500 p-4">
            ...
            <div className="relative">
                <Input
                    ...
                    value={value}
                    onChange={(e) => handleOtherTenantsInputChange(e.currentTarget.value)}
                    onFocus={() => {
                        if (!disabled) {
                            setFilteredTenants(tenants);
