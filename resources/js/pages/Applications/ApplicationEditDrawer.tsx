...
export default function ApplicationEditDrawer({ 
    application,
    units, 
    cities, 
    properties, 
    ...
    useEffect(() => {
        if (application) {
            const currentUnit = units.find(u => u.property === application.property && u.unit_name === application.unit);
            const cityToUse = application.city || currentUnit?.city || '';
            
            setSelectedCity(cityToUse);
            
            // Set available properties
            if (cityToUse && properties[cityToUse]) {
                setAvailableProperties(properties[cityToUse]);
            } else {
                setAvailableProperties([]);
            }
            
            // Set available units
            if (cityToUse && application.property && unitsByProperty[cityToUse]?.[application.property]) {
                setAvailableUnits(unitsByProperty[cityToUse][application.property]);
            } else {
                setAvailableUnits([]);
            }
    ...
    }, [application, units, properties, unitsByProperty]);
